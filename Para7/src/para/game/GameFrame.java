...
public class GameFrame extends Application {
   ...
   * このクラスでは、左右のボタン(F キー と J
   * キー)が押されているかを標準出力に出力する 処理が例として定義されている。
  ...
  private void setLeft() { lefton = 1; }
  private void unsetLeft() { lefton = 0; }
  private void setRight() { righton = 1; }
  private void unsetRight() { righton = 0; }

  void setState(int state) { this.state = state; }

  void setGamerState(int state) { this.gamerstate = state; }

  /**
   * JavaFXアプリケーション・スレッドから呼び出されるメソッド.
   * ウィンドウを開き、待機状態とする
   * @param stage
   * JavaFXアプリケーション・スレッドから渡される最上位のJavaFXコンテナ
   */
  public void start(Stage stage) {
    ...
    button.setOnAction(ev -> { gamestart(spinner.getValue()); });
